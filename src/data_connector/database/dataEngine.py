...
from .. import dcConstants as dcc
...
class DataEngine(object):
	...
	"""
	def __init__(self, corpdb=dcc.DEF_CORPDB, mysql_config_file=dcc.MYSQL_CONFIG_FILE, encoding=dcc.DEF_ENCODING,use_unicode=dcc.DEF_UNICODE_SWITCH, db_type=dcc.DB_TYPE):
...
class MySqlDataEngine(DataEngine):
	...
	def standardizeTable(self, table, collate, engine, charset, use_unicode):
		...
		True if the query execution is successful 
		"""
		return mm.standardizeTable(self.corpdb, self.dbCursor, table, collate=dcc.DEF_COLLATIONS[self.encoding.lower()], engine=dcc.DEF_MYSQL_ENGINE, charset=self.encoding, use_unicode=self.use_unicode, mysql_config_file=self.mysql_config_file)
