...
def get_db_engine(db_schema, charset=dcc.DEF_ENCODING, mysql_config_file=dcc.MYSQL_CONFIG_FILE, port=3306, stream=False):
    eng = None
    attempts = 0;
    while (1):
        try:
            ...
            if stream:
                eng = create_engine(db_url, connect_args={'cursorclass': pymysql.cursors.SSCursor}, pool_recycle=600)
            else:
            ...
            time.sleep(dcc.MYSQL_ERROR_SLEEP*attempts**2)
            if (attempts > dcc.MAX_ATTEMPTS):
                sys.exit(1)
    return eng
