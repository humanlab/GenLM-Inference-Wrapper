...
from data_connector.dcConstants import MAX_ATTEMPTS, MYSQL_ERROR_SLEEP, DEF_ENCODING, MAX_SQL_PRINT_CHARS, DEF_UNICODE_SWITCH, DEF_MYSQL_ENGINE, MYSQL_CONFIG_FILE, warn, DEF_COLLATIONS
...
def dbConnect(db, charset=DEF_ENCODING, use_unicode=DEF_UNICODE_SWITCH, mysql_config_file=MYSQL_CONFIG_FILE):
    ...
    dictCursor = dbConn.cursor(pymysql.cursors.DictCursor)
...
def executeGetSSCursor(db, sql, warnMsg = True, charset=DEF_ENCODING, use_unicode=DEF_UNICODE_SWITCH, mysql_config_file=MYSQL_CONFIG_FILE):
    ...
    ssCursor = dbConnect(db, 
                         ...
                         )[0].cursor(pymysql.cursors.SSCursor)
    data = []
    attempts = 0;
    while (1):
        try:
            ...
            ssCursor = dbConnect(db, charset=charset, use_unicode=use_unicode, mysql_config_file=mysql_config_file)[0].cursor(pymysql.cursors.SSCursor)
            if (attempts > MAX_ATTEMPTS):
                sys.exit(1)
    return ssCursor

def abstractDBConnect(db, mysql_config_file):
    dbConn = pymysql.connect (db = db,
                          read_default_file = mysql_config_file
                          )
    dbCursor = dbConn.cursor()
